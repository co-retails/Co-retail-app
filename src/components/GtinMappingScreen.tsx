...
export function GtinMappingScreen({ onBack }: GtinMappingScreenProps) {
  ...
  const handleSaveInlineEdit = (mapping: GtinMapping) => {
    ...
    if (mapping.source === 'inherited' && filterChip === 'inherited') {
      // Editing inherited defaults is not yet supported (derived from partner type configs)
      toast.error('Editing inherited mappings is not available yet');
      setEditingRowId(null);
      setInlineEditState(null);
      return;
    } else {
  ...
  const handleSave = () => {
    ...
    if (dialogMode === 'edit' && editingMapping) {
      ...
      if (filterChip === 'inherited' || (!formState.partnerId && dialogMode === 'create' && !editingMapping)) {
        // Creating inherited defaults is not yet supported (derived from partner type configs)
        toast.error('Creating inherited mappings is not available yet');
        setIsDialogOpen(false);
        resetForm();
        return;
      } else {
  ...
  const handleDeleteSelected = () => {
    ...
    if (filterChip === 'inherited') {
      // Deleting inherited defaults is not yet supported (derived from partner type configs)
      toast.error('Deleting inherited mappings is not available yet');
      return;
    }
    ...
    // Only overrides can be deleted; inherited mappings are read-only for now
    const deletableIds = new Set(
      [...selectedRows].filter(id => partnerOverrides.some(m => m.id === id))
    );
    setPartnerOverrides(prev => prev.filter(m => !deletableIds.has(m.id)));

    const skipped = selectedRows.size - deletableIds.size;
    if (deletableIds.size > 0) {
      toast.success(`${deletableIds.size} override(s) deleted`);
    }
    if (skipped > 0) {
      toast.error(`${skipped} inherited mapping(s) skipped — not deletable yet`);
    }
    setSelectedRows(new Set());
  };
      ...
      reader.onload = (event) => {
        try {
          ...
          dataRows.forEach((row: any) => {
            ...
            // Only overrides (rows with a Partner ID) can be imported for now.
            // Inherited defaults are derived from partner type configs and not yet editable.
            if (partnerId) {
              const newMapping: GtinMapping = {
          ...
          if (imported > 0) {
            toast.success(`Imported ${imported} override(s)`);
          }
          if (skipped > 0) {
            toast.error(`${skipped} inherited mapping(s) skipped — import not available yet`);
          }
        } catch (error) {
  ...
  const handleStartEditPartnerType = (partnerType: PartnerType) => {
    setEditingPartnerTypeName(partnerType);
    setEditingPartnerTypeValue(partnerType);
  };

  const handleCancelEditPartnerType = () => {
    setEditingPartnerTypeName(null);
    setEditingPartnerTypeValue('');
  };

  const handleSaveEditPartnerType = () => {
    const original = editingPartnerTypeName;
    const next = editingPartnerTypeValue.trim() as PartnerType;
    if (!original) return;
    if (!next) {
      toast.error('Partner type name is required');
      return;
    }
    if (next !== original && availablePartnerTypes.includes(next)) {
      toast.error('Partner type already exists');
      return;
    }
    if (next !== original) {
      setAvailablePartnerTypes(prev => prev.map(pt => (pt === original ? next : pt)));
      setPartnerTypeConfigs(prev =>
        prev.map(config => (config.partnerType === original ? { ...config, partnerType: next } : config))
      );
    }
    setEditingPartnerTypeName(null);
    setEditingPartnerTypeValue('');
  ...
  const handleDeletePartnerType = (partnerType: PartnerType) => {
    setAvailablePartnerTypes(prev => prev.filter(pt => pt !== partnerType));
    setPartnerTypeConfigs(prev => prev.filter(config => config.partnerType !== partnerType));
    if (editingPartnerTypeName === partnerType) {
      setEditingPartnerTypeName(null);
      setEditingPartnerTypeValue('');
    }
