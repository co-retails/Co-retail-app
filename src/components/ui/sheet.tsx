...
>(({ className, children, side = "right", ...props }, ref) => {
  ...
  const hasDescription = React.Children.toArray(children).some(
    (child: any) => {
      // Check for direct SheetDescription component
      if (child?.type?.displayName === 'SheetDescription') return true;
      
      // Check for data-slot attribute
      if (child?.props?.["data-slot"] === 'sheet-description') return true;
      
      // Check if it's a SheetPrimitive.Description
      if (child?.type === SheetPrimitive.Description) return true;
      
      // Check nested children (for SheetHeader containing SheetDescription)
      if (child?.props?.children) {
        const nestedChildren = React.Children.toArray(child.props.children);
        return nestedChildren.some((nestedChild: any) => 
          nestedChild?.type?.displayName === 'SheetDescription' ||
          nestedChild?.props?.["data-slot"] === 'sheet-description' ||
          nestedChild?.type === SheetPrimitive.Description
        );
      }
  ...
  const getAriaDescribedBy = () => {
    ...
    // If there's a SheetDescription child, don't set aria-describedby (let Radix handle it)
    if (hasDescription) {
      return undefined;
    }
    
    // Otherwise, use our fallback description
    return "sheet-fallback-description";
  };

  const ariaDescribedBy = getAriaDescribedBy();
  const shouldShowFallback = !hasDescription && !props["aria-describedby"];
  
  // Desktop style for right-side sheets - M3 guidelines
  const desktopStyle = isDesktop && effectiveSide === "right" ? {
    width: '33vw',
    maxWidth: '400px',
    minWidth: '360px'
  } : undefined;
    ...
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ...
        className={cn(
          ...
          !isDesktop && effectiveSide === "right" && "w-3/4 sm:max-w-sm",
          effectiveSide === "left" &&
